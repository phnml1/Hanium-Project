...
function MainPage() {
  ...
  useEffect(()=>{axiosInstance.get('/jwt/info',{headers:{Authorization: `Bearer ${localStorage.getItem('user')}`}})})
  return (
          ...
          {featureMenus.map((a, i) => {
            return (
              <div
                ...
                className={`group w-40 cursor-pointer h-48 scale-100 flex flex-col justify-center items-center ease-in-out duration-300 rounded-lg hover:scale-110`}
              >
                <div className="w-full text-center text-lg">
                  {a.name}
                  <div className="text-sm ease-in-out opacity-0 -translate-y-2 duration-200 group-hover:translate-y-0 group-hover:opacity-100">
                    {a.kor}
                  </div>
                </div>
              </div>
        ...
        <div className="w-5/6 h-52 my-10 relative items-center bg-white rounded-md">
          <div className="w-fit flex gap-6 absolute left-16 top-8">
            ...
            <div className="text-xl">
              D37번 컨베이어
              <div className="text-sm text-gray-500">번호: WS1023910</div>
            </div>
          </div>
